...
메서드(method)
 	...
 	생성자 : 파일명과 클래스이름과 똑같이 작성을 해줘야하고 객체의 기본 값을 설정하거나 값을 각각 변경해서 사용하는데 쓰이는 메서드
 		 ...
 		 public 클래스이름(자료형 매개변수1, 자료형 매개변수2){
 			...
 			예를들어 회원가입을 진행할 때 필수로 이름 나이 는 넣어주고
 					결혼기념일 광고수신동의는 선택으로 하고 싶다면 필수로 이름 나이 를( ) 넣어줌
 		...
 		void : 어떤 행동을 수행하기 위해 작성하고, 수행한다음 특별히 전달할 내용이 없을 때 사용
 				...
 				특정 행동을 하고 난 다음 행동을 끝내는 것이지 값을 전달하거나 추가로 어떤 행동을 하지 않음
 		...
 		return : 어떤 행동을 실행하고 행동을 한 다음에 행동에 대한 결과를 전달
 			...
 			//문자열을 전달 = String
 			  public String 특정행동1 () {
 			  	return "안녕하세요"; // 위에 String을 작성했으니 return String을 작성
 			  }
 			  public int 특정행동2() {
