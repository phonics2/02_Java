...
/*
 ...
 자료형(참조) = Reference = 레퍼런스
 	문자열 : String				// "" 사용 문자를 모두 나열하는 문자열 "" 특수문자 등 모든 것 사용가능
 
 상수형		
 	final	자료형	상수명 = 값;
 	final int MAX_NUM = 100; 	// 상시적으로 변하지 않는 수 값 변화x 이름을 대문자로 표기
...
//필드
	//자료형 기본
		//1. 숫자
			...
			long  lng = 5;
		//2. 실수
			float  ft = 3.14f; // 기본 실수 값은 double 이고 double은 float 보다 큰 값이 때문에
		...
		// boolean 은 기본값이 false 이기때문에 
		 boolean bln; //boolean bln = false; 는 서로 같은뜻
