...
public class TestDate3 {
	...
	//본인의 생년 월일 설정
		...
		//현재 날짜 구하기
		Calendar 현재날 = Calendar.getInstance();
		int 현재년도 = 현재날.get(Calendar.YEAR);
		int 현재월 = 현재날.get(Calendar.MONTH)+1;
		int 현재날짜 = 현재날.get(Calendar.DAY_OF_MONTH);
		...
		System.out.println("현재 : " + 날짜예쁘게작성.format(현재날.getTime()));
